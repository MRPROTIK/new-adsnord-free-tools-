...
/** ✅ CORS allowlist (only for browsers with Origin header) */
 ...
 * - If Origin exists and allowed -> reflect it
 * - If Origin missing -> don't set ACAO (same-origin/non-browser calls still work)
 */
...
function buildKeywords(keyword: string, business: string, goal: string) {
  ...
  const short = [`${base} ${b}`.trim(), `${base} ${g}`.trim(), `${base} service`.trim()];
  const long = [`hire ${base} ${b} for ${g}`.trim(), `best ${base} ${b} to get ${g}`.trim()];

  const uniq = Array.from(new Set([...short, ...long])).slice(0, 5);

  return uniq.map((k, idx) => {
...
async function rateLimit(env: Env, ip: string): Promise<{ ok: boolean; retryAfterSec?: number }> {
  ...
  await env.AN_KV.put(key, String(count + 1), { expirationTtl: RL_WINDOW_SEC + 5 });
  return { ok: true };
}
...
function validateInputs(keyword: string, business: string, goal: string) {
  ...
  if (business.length < 2 || business.length > 50) return { ok: false, code: "BAD_BUSINESS", message: "Business value is invalid." };
  if (goal.length < 2 || goal.length > 40) return { ok: false, code: "BAD_GOAL", message: "Goal value is invalid." };
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "OPTIONS") {
      // If browser sends Origin, only allow your site
    ...
    /** ✅ Block only when Origin exists AND is not allowed */
    if (origin && !ALLOWED_ORIGINS.has(origin)) {
    ...
    if (!v.ok) return error(request, 400, v.code || "BAD_REQUEST", v.message || "Invalid inputs.");
    ...
    const payload = { status: "success", version: VERSION, results, best_pick, copy_block };
